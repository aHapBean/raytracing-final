...
//为了右边的贴图
class zy_rect : public hittable {
public:
    zy_rect() {}

    zy_rect(double _z0, double _z1, double _y0, double _y1, double _k, shared_ptr<material> mat)
        : z0(_z0), z1(_z1), y0(_y0), y1(_y1), k(_k), mp(mat) {};
...
bool zy_rect::hit(const ray& r, double t0, double t1, hit_record& rec) const {
    auto t = (k - r.origin().x()) / r.direction().x();
    if (t < t0 || t > t1)
        return false;
    auto z = r.origin().z() + t * r.direction().z();
